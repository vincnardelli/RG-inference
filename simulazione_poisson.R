...
simulazione_poisson <- function(n=80, u_sigma=1){
  ...
  u=rnorm(n,0,u_sigma)
  w=x+u
  glm(y ~ w, family = poisson(link = log))
...
simulazione_lista <- parallel::mclapply(1:nrow(simulazione), function(x) simulazione_poisson(n=80, u_sigma=simulazione[x, ]$u_sigma), 
                                        mc.cores = parallel::detectCores()-1)


# Versione non parallela
# simulazione_lista <- lapply(1:nrow(simulazione), function(x) simulazione_poisson(n=80, u_sigma=simulazione[x, ]$u_sigma))
...
beta0 | beta1
ggsave("figures/poisson_stima1.pdf", beta0 | beta1, height = 4, width = 8)
...
beta0 | beta1
ggsave("figures/poisson_stima2.pdf", beta0 | beta1, height = 4, width = 8)
...
beta0_se | beta1_se
ggsave("figures/poisson_se.pdf", beta0_se | beta1_se, height = 4, width = 8)
...
beta1_test
ggsave("figures/poisson_potenza.pdf", beta1_test, height = 4, width = 8)
