...
simulazione_real$beta0_lower=unlist(sapply(simulazione_real_lista, function(x) x[1,1]))
simulazione_real$beta0_upper=unlist(sapply(simulazione_real_lista, function(x) x[1,2]))

simulazione_real$beta1_lower=unlist(sapply(simulazione_real_lista, function(x) x[2,1]))
simulazione_real$beta1_upper=unlist(sapply(simulazione_real_lista, function(x) x[2,2]))

simulazione_real$beta2_lower=unlist(sapply(simulazione_real_lista, function(x) x[3,1]))
simulazione_real$beta2_upper=unlist(sapply(simulazione_real_lista, function(x) x[3,2]))

simulazione_real$beta3_lower=unlist(sapply(simulazione_real_lista, function(x) x[4,1]))
simulazione_real$beta3_upper=unlist(sapply(simulazione_real_lista, function(x) x[4,2]))
...
risultati <- simulazione %>% 
  ...
  pivot_longer(lower:upper, names_to="bound", values_to = "estimate") %>%
  left_join(simulazione_real, by=c("dataset", "tt", "parameter", "bound")) %>%
  filter(prop != 0) %>%
  group_by(dataset, prop, tt, parameter) %>%
  pivot_wider(names_from = bound, values_from = estimate:rq)
...
ggplot() + 
  geom_ribbon(aes(x= prop, 
                  ymin = estimate_lower, 
                  ymax = estimate_upper), fill="gray80") +
  geom_hline(aes(yintercept = rq_lower)) +
  geom_hline(aes(yintercept = rq_upper)) +
